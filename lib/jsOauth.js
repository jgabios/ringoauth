 ...
 *  you can find it here: https://github.com/bytespider/jsOAuth/
 *  this copy here has been modified to be used in a ringojs environment
 */
var exports = exports || this;
var httpclient = require('ringo/httpclient');
var File = require('fs').Path;
var {jsDump} = require('test');
    ...
    OAuth.prototype = {
        ...
        init: function (options) {
            ...
            this.request = function (options) {
                var method, url, async, data, headers, success, failure, i,
                ...
                var onComplete = function (content, status, contentType, exchange) {
                            var requestHeaders = headers,
                            responseHeaders = exchange.headers;
			//print('i got response: ' + content);
                        var includeXML = false;
                        if ('Content-Type' in responseHeaders)
                        {
                            if (responseHeaders['Content-Type'].indexOf('text/xml') != -1)
                            {
                                includeXML = true;
                            }

                        }
                        var responseObject = {text: content, xml: (includeXML ? content : ''), requestHeaders: requestHeaders, responseHeaders: responseHeaders};

                        // we are powerless against 3xx redirects
                        if((status >= 200 && status <= 226) || status == 304 || status === 0) {
                ...
                if(async){
                    ...
                    var exchange = httpclient.request({
                     ...
                     headers: headers
                        ...
                        var responseHeaders = exchange.headers;
			//print('i got response: ' + content);
                        var includeXML = false;
                        debugger;
                        if ('Content-Type' in responseHeaders)
                        ...
                        var responseObject = {text: exchange.content, xml: (includeXML ? exchange.content : ''), requestHeaders: headers, responseHeaders: responseHeaders};
         ...
         * @param sync {boolean} if you want the request to be synchronous
         */
        get: function (url, success, failure, sync) {
            this.request({'url': url, 'success': success, 'failure': failure, 'async': sync === true ? false : true});
        },
         ...
         * @param sync {boolean} if you want the request to be synchronous
         */
        post: function (url, data, success, failure, sync) {
            this.request({'method': 'POST', 'url': url, 'data': data, 'success': success, 'failure': failure, 'async': sync === true ? false : true});
        },
         ...
         * @param sync {boolean} if you want the request to be synchronous
         */
        getJSON: function (url, success, failure, sync) {
            this.get(url, function (data) {
                success(JSON.parse(data.text));
            }, failure, sync === true ? false : true);
        },
         ...
         * @param sync {boolean} if you want the request to be synchronous
         */
        postJSON: function (url, data, success, failure, sync) {
            this.request({
                'method': 'POST',
                'url': url,
        ...
        fetchRequestToken: function (success, failure, sync) {
            var oauth = this;
        ...
        fetchAccessToken: function (success, failure, sync) {
            var oauth = this;
